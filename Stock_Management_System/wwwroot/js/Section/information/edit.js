...
function CheckData() {
    ...
    if (missingFields.length > 0) {
        ...
        confirmupdateBankInformation('/Information/UpdateBankInformationDetails');
        return false; // Prevent form submission until confirmation and AJAX call are completed
    }
}

function confirmupdateBankInformation(redirectUrl) {
    Swal.fire({
        title: 'Confirm Edit Bank Information',
        text: 'Are you sure you want to Update this bank information?',
        icon: 'question',
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function (response) {
                    sessionStorage.setItem('AddBankStatus', 'Bank information Update successfully!');
